...
if (isset($_POST['closeID']))
{
	...
	if(mysql_num_rows($checkID)==0){
		$query = "INSERT INTO tabclosecashierheader (closeID,closeDate,closePeriode,closeShift,outletID,dpp,vat,grandTotal,totalReceived,remarks,userID,status,dateCreated,lastChanged) VALUES('$closeID', '$closeDate','$closePeriode','$closeShift','$outletID','$dpp','0','$grandTotal','$totalReceived','$remark','$user', '1','$dateCreated', '$lastChanged')";
		...
		$queryDetail = mysql_query("SELECT h.orderID, h.orderNo, h.orderDate, h.orderPeriode, h.orderAmount, p.total, p.paymentAmount, p.paymentMethod, h.payerName, p.discountPrice, p.promoID, m.methodName, h.userID, u.fullname, h.remarks FROM taborderheader h 
		...
		while($rows = mysql_fetch_array($queryDetail)){
			...
			$insertDetail = mysql_query("INSERT INTO tabclosecashierdetail (id,orderID,closeID,dpp,VAT,discountPrice,total,promoID,paymentAmount,paymentMethod,payerName,remarks,userID,status,dateCreated,lastChanged) VALUES ('$id','$orderID','$closeID','$dpp','0','$discountPrice','$totalReceived','$promoID','$paymentAmount','$paymentMethod','$payerName','$remark','$user','1','$dateCreated','$lastChanged')");
		}
