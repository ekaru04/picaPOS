...
<div>
    ...
    <div class="container-fluid">
        <table class="mb-3" id='searchDateBox2'>
          <tr>
...
<hr class="mt-4 mb-4">
      ...
      $query = mysql_query("SELECT c.categoryID, c.categoryName FROM mcategory c WHERE c.status = 1");
...
$(document).ready(function(){
  var dateA = $("#dateA").val('<?php echo date("Y-m-d");?>');
  var dateB = $("#dateB").val('<?php echo date("Y-m-d");?>');
  var status = $("#status").val('');
  $("#searchDate2").click(function(){searchDateJournal();}); 
  // alert('dateA');
});
...
var itemTable = $(_categoryIdArr.name).DataTable(
    {
        ...
        responsive : true,
        ajax: {
            url: "categoryA.php?categoryID="+_categoryIdArr.id+"&dateA="+"<?php echo date('Y-m-d');?>"+"&dateB="+"<?php echo date('Y-m-d');?>",
            data: 'data'
        },
        columns: [
            { data: 'no' },
...
$('#searchDate2').click(function(e){
    ...
    var endDateA = new Date($('#dateB').val());
        ...
        var urlS2 = "prices.php?dateA="+$("#dateA").val()+"&dateB="+$("#dateB").val();
...
{
  var startDate = new Date($('#dateA').val());
  var endDate = new Date($('#dateB').val());
  if (startDate <= endDate){
    for(var _dataTable of dataTableCategory){
    var urlS = "categoryFilter.php?categoryID="+_dataTable.id+"&dateA="+$("#dateA").val()+"&dateB="+$("#dateB").val();
    _dataTable.ajax.url(urlS).load();
  }
  }else{
